...
| 场景 | 搜索模式 | 脚本 | 优势 |
|------|----------|------|------|
| 用户说了模糊需求（"做个演示"） | **混合搜索** | `search_hybrid("做个演示")` | 向量+FTS5+LIKE 三路召回 |
...
# === 模式1：语义搜索（默认推荐） ===
# 用户说模糊需求时优先使用
python ~/.claude/skills/skill-advisor-skill-recommender/scripts/search_hybrid() "做个汇报用的幻灯片"
...
1. **优先用语义搜索** —— 当用户说模糊需求时（"做个演示"），用 `search_hybrid()` 而非关键词匹配
...
| `search_hybrid()` | TF-IDF 语义搜索 + 同义词扩展 | **模糊需求首选** |
